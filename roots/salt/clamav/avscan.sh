...
scan() {
  ...
  if [ ! -f "$1/.fullavscan" ]; then
    ...
    find $1 -xdev -mmin ${TIME_INTERVAL} \( ! -wholename $1/backups.txt ! -wholename $1/.zsh_history ! -wholename $1 ! -wholename $1/avscan.txt \) -print0 | xargs -0 -r clamscan -l "$1/avscan.txt" ${PARAMETERS} >> /var/log/clamav/avscan.log
  fi
}
